...
| D9 | Delete | Whole-asset delete only, behind a More menu. Confirmation is the tool's own window (a message box cannot carry a choice): it names the asset, says the removal cannot be undone, and offers one opt-in, off every time — **also delete the content I added in plugin `<name>`**, which deletes the Source Packages under that plugin's mount root but keeps its Primary Asset Label so the Chunk survives. Version-level delete stays a script-only Command. **Amended** for ar-convai/ConvaiTask#220 follow-up. |
| D10 | Post-delete | On whole-asset delete success the subsystem deletes every record of that Asset — `CreateAssetData`, `PakMetaData`, `Thumbnail` and `RawArchive` — so the Chunk returns to Draft with an **empty** form. `ModdingMetaData` stays: it describes the project (plugin, Asset Type), not the Asset, and nothing regenerates it. **Supersedes the original D10**, which kept name/description/thumbnail/entry point. |
